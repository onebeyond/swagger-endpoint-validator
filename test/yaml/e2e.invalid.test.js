...
describe('validator', () => {
	it('should throw \'swagger_validator\' error when sending invalid empty input payload', done => {
		...
		app.invoke('post', '/test-invalid-input', req, res);
	});

	it('should throw \'swagger_validator\' error when sending invalid empty output payload', done => {
		const req = app.makeRequest();
		const res = app.makeResponse((err, sideEffects) => {
	...
	it('should throw \'swagger_validator\' error and descriptive error when sending invalid output payload', done => {
		const req = app.makeRequest();
		const res = app.makeResponse((err, sideEffects) => {
			// This is necessary because of the side Effects of the library
			if (sideEffects.json && sideEffects.json.error.type === 'swagger_validator') {
		...
		app.invoke('get', '/test-invalid-output-2', req, res);
	});
});
