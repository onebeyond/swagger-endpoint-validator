...
const init = async (app, options, spec) => {
	...
	await new OpenApiValidator({
		...
		// ignorePaths: /.*\/api\/v1\/test\/schema$/,
...
const validateSchema = body => {
	...
	if (!endpoint) {
		throw SwaggerValidatorError('Required \'endpoint\' missing in requestBody');
	}
	if (!method) {
		throw SwaggerValidatorError('Required \'method\' missing in requestBody');
	}
	if (!fixture) {
		throw SwaggerValidatorError('Required \'fixture\' missing in requestBody');
	}
